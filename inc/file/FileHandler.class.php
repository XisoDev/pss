...
class FileHandler
{
	...
	function getRealPath($source)
	{
		...
		{
			return _XISO_PATH_ . substr($source, 2);
		}
	...
	function readDir($path, $filter = '', $to_lower = FALSE, $concat_prefix = FALSE)
	{
		...
		foreach($files as $file)
		{
			...
			if($concat_prefix)
			{
				$file = sprintf('%s%s', str_replace(_XISO_PATH_, '', $path), $file);
			}
	...
	function makeDir($path_string)
	{
		...
		else
		{
			...
			$ftp_info->ftp_user = 'softmaster';
			$ftp_info->ftp_password = 'rptdjq1djr';
			$ftp_info->ftp_port = '21';
			$ftp_info->ftp_host = '127.0.0.1';
			$ftp_info->ftp_pasv = 'N';
			$ftp_info->sftp = 'N';
			$ftp_info->ftp_root_path = '/home/softmaster/';
			...
			if($oFtp == NULL)
			{
				...
				require_once(_XISO_PATH_ . 'libs/ftp.class.php');
				$oFtp = new ftp();
			...
			$path_string = str_replace(_XISO_PATH_, '', $path_string);
	...
	{
		require_once(_XISO_PATH_ . 'libs/idna_convert/idna_convert.class.php');
		...
		try
		{
			...
			else
			{
				...
				if(method_exists($oRequest, 'setConfig'))
				{
					...
					if(file_exists(_XISO_PATH_ . 'libs/cacert/cacert.pem'))
					{
						$oRequest->setConfig('ssl_cafile', _XISO_PATH_ . 'libs/cacert/cacert.pem');
					}
				}
